...
namespace net.roomful.assets {
	...
	public class MeshSerializer {
		...
		static void ReadVector3Array16Bit(Vector3[] arr, BinaryReader buf) {
			...
			for (var i = 0; i < n; ++i)
			{
				var ix = buf.ReadUInt16();
				var iy = buf.ReadUInt16();
				var iz = buf.ReadUInt16();
				var xx = ix / 65535.0f * (bmax.x - bmin.x) + bmin.x;
				var yy = iy / 65535.0f * (bmax.y - bmin.y) + bmin.y;
				var zz = iz / 65535.0f * (bmax.z - bmin.z) + bmin.z;
		...
		static void WriteVector2Array16Bit(Vector2[] arr, BinaryWriter buf) {
			...
			// Calculate bounding box of the array
			var bmin = arr[0] - new Vector2(0.001f, 0.001f);
			var bmax = arr[0] + new Vector2(0.001f, 0.001f);
			foreach (var v in arr)
			{
		...
		static void ReadVector3ArrayBytes(Vector3[] arr, BinaryReader buf) {
			...
			for (var i = 0; i < n; ++i)
			{
				...
				var iy = buf.ReadByte();
				var iz = buf.ReadByte();
				var xx = (ix - 128.0f) / 127.0f;
				var yy = (iy - 128.0f) / 127.0f;
				var zz = (iz - 128.0f) / 127.0f;
				arr[i] = new Vector3(xx, yy, zz);
			}
		}
		...
		public static string SerializerMesh(GameObject origin) {
			...
			var activeState = origin.activeSelf;
