...
namespace net.roomful.assets {
	...
	public class ThumbnailSilhouette  {
        ...
        public ThumbnailSilhouette(PropThumbnail thumbnail) {
			...
			if(thumbnail.Frame != null) {
				...
				if(thumbnail.Frame.Border != null && thumbnail.Frame.Corner != null) {
                    var border = Object.Instantiate(thumbnail.Frame.Border, Vector3.zero, Quaternion.identity);
                    var corner = Object.Instantiate(thumbnail.Frame.Corner, Vector3.zero, Quaternion.identity);
		...
		private void ParseTemplate(JSONData thumbnailInfo) {
            ...
            var MobileGeometryPosition = new JSONData(thumbnailInfo.GetValue<Dictionary<string, object>>("position"));
			...
			var MobileGeometryRotation = new JSONData(thumbnailInfo.GetValue<Dictionary<string, object>>("rotation"));
			...
			if(thumbnailInfo.HasValue("scale")) {
				var MobileGeometryScale = new JSONData(thumbnailInfo.GetValue<Dictionary<string, object>>("scale"));
