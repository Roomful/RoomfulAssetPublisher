...
- (NSDictionary*)getLogFileNames {
    ...
    NSError *error;
    NSArray *files = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:_logDirectoryPath error:&error];
    ...
    if (!error) {
        for (NSString* file in [files reverseObjectEnumerator]) {
            if ([file containsString:appName]) {
                [globalLogFileNames addObject:file];
            }
        }
