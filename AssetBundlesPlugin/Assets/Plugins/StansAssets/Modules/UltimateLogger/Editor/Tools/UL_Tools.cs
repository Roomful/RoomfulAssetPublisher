...
namespace SA.UltimateLogger {
	...
	public class UL_Tools {
		...
		public static Texture2D SALogo {
			get {
				if(_SALogo == null) {
					...
					string path = "Assets/" + EditorUtils.SUPPORT_MODULS_PATH + "Common/Editor/Content/";
		...
		public static void DrawSALogo() {
			...
			if(click) {
				Application.OpenURL(EditorUtils.WEBSITE_ROOT_URL);
			}
		}
		...
		public static bool ToggleFiled(GUIContent title, bool value) {
			...
			UL_Bool initialValue = UL_Bool.Enabled;
			if(!value) {
				initialValue = UL_Bool.Disabled;
			}
		//	EditorGUILayout.BeginHorizontal();
		//	EditorGUILayout.LabelField(title);
			
			initialValue = (UL_Bool) EditorGUILayout.EnumPopup(title, initialValue);
			if(initialValue == UL_Bool.Enabled) {
				value = true;
			} else {
				value = false;
			}
		...
		public static bool YesNoFiled(GUIContent title, bool value) {
			...
			UL_YesNoBool initialValue = UL_YesNoBool.Yes;
			if(!value) {
				initialValue = UL_YesNoBool.No;
			}
			EditorGUILayout.BeginHorizontal();
			EditorGUILayout.LabelField(title);

			initialValue = (UL_YesNoBool) EditorGUILayout.EnumPopup(initialValue);
			if(initialValue == UL_YesNoBool.Yes) {
				value = true;
			} else {
				value = false;
			}
		...
		public static void ChnageDefineState(string file, string tag, bool IsEnabled) {
			if(EditorUtils.IsFileExists(file)) {
				string content = EditorUtils.Read(file);
				//	Debug.Log(file);
				//Debug.Log(content);
				...
				EditorUtils.Write(file, content);
			}		
		}
		...
		public static void SupportMail() {
			SelectableLabelField(SupportEmail, EditorUtils.SUPPORT_EMAIL);
		}
