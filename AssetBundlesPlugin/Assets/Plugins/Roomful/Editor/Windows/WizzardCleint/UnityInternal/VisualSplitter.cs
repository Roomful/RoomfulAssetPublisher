    ...
    internal class VisualSplitter : VisualElement
    {
        ...
        protected override void DoRepaint(IStylePainter painter) {
            ...
            for (int i = 0; i < shadow.childCount - 1; ++i) {
                ...
                var rect = GetSplitterRect(visualElement);
                EditorGUIUtility.AddCursorRect(rect, isVertical ? MouseCursor.ResizeVertical : MouseCursor.SplitResizeLeftRight);

                var splitterDrawRect = new Rect(rect);
                splitterDrawRect.x += 3;
                splitterDrawRect.width -= 6;
                GUI.Box(splitterDrawRect, new GUIContent(""));

                //wondering why this and the cursor diesnt really work.
