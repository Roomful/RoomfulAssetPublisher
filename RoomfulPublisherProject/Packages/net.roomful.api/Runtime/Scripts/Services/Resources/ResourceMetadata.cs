...
namespace net.roomful.api {
	...
	public class ResourceMetadata {
        ...
        public static readonly List<string> VIDEO_TYPES = new List<string> { MimeTypeHlsStream, MimeTypeDashStream, MimeTypeYoutube, "video/mp4", "video/avi", "video/mpeg", "video/ogg", "video/quicktime", "video/webm"  };
		...
		public const string MimeTypeHlsStream = "application/x-mpegurl";
		public const string MimeTypeDashStream = "application/dash+xml";
		...
		public string ConvertedFromMimeType;
		public string Link = string.Empty;
		public string BehaviourType = string.Empty;
		...
		public Dictionary<string, object> ToDictionary() {
			...
			data.Add("convertedFrom", ConvertedFromMimeType);
			if(!Link.Equals (string.Empty)) {
				data.Add("link", Link);
			}
			data.Add("behaviourType", BehaviourType);
		...
		private void ParsePropMetaData(JSONData metaInfo) {
			...
			if (metaInfo.HasValue("behaviourType"))
				BehaviourType = metaInfo.GetValue<string>("behaviourType");

			if(metaInfo.HasValue("convertedFrom"))
				ConvertedFromMimeType = metaInfo.GetValue<string>("convertedFrom");
        ...
        public bool IsVideoStreamMimeType => MimeType.Equals(MimeTypeHlsStream) || MimeType.Equals(MimeTypeDashStream);
	}
}
