...
public class LakePolygon : MonoBehaviour
{
    ...
    public static LakePolygon CreatePolygon(Material material, List<Vector3> positions = null) {
        return CreatePolygon<LakePolygon>(material, positions);
    }

    public static LakePolygon CreatePolygon<T>(Material material, List<Vector3> positions = null) where T : LakePolygon {
        GameObject gameobject = new GameObject(typeof(T).Name);
        gameobject.layer = LayerMask.NameToLayer("Water");

        LakePolygon polygon = gameobject.AddComponent<T>();
