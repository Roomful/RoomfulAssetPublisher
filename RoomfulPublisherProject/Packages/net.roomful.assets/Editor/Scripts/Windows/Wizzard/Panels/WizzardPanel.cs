...
namespace net.roomful.assets.editor
{
    internal class WizzardPanel : Panel
    {
        ...
        private List<IAssetWizzard> Wizards {
            get {
                if (m_wizards == null || m_wizards.Count == 0) {
                    ...
                    m_wizards.Add(new PropWizard());
                    m_wizards.Add(new EnvironmentWizard());
                    m_wizards.Add(new StyleWizard());
                }
        ...
        private void NoAssetWizard() {
            ...
            var environment_icon = IconManager.GetIcon(Icon.environment_icon_light);
            if (EditorGUIUtility.isProSkin) {
                environment_icon = IconManager.GetIcon(Icon.environment_icon);
            }

            var style_icon = IconManager.GetIcon(Icon.style_icon_light);
            if (EditorGUIUtility.isProSkin) {
        ...
        private void DrawCreateAssetItem(string title, string msg, Texture2D image, System.Action callback) {
            ...
            var createAssetContent = new GUIContent();
