...
namespace net.roomful.assets.editor
{
    ...
    internal class PropAssetInspector : AssetInspector<PropAssetTemplate, PropAsset>
    {
        ...
        void OnEnable() {
            ...
            SceneViewOptionsWindow.RegisterPropAsset((PropAsset) target);
        }
        ...
        public override void OnInspectorGUI() {
            ...
            PropAssetSettings extendedSettings;
            using (new IMGUIBeginHorizontal()) {
                GUILayout.FlexibleSpace();
                extendedSettings = Asset.GetLayer(HierarchyLayers.Graphics).GetComponent<PropAssetSettings>();
                var extendedSettingsButtonTitle = "Add Extended Setting";
                if (extendedSettings != null) {
                    extendedSettingsButtonTitle = "Remove Extended Setting";
                }

                var extendedSettingsButton = GUILayout.Button(extendedSettingsButtonTitle, EditorStyles.miniButton, GUILayout.Width(180));
                if (extendedSettingsButton) {
                    if (extendedSettings != null) {
                        DestroyImmediate(extendedSettings);
                    }
                    else {
                        Asset.GetLayer(HierarchyLayers.Graphics).gameObject.AddComponent<PropAssetSettings>();
                    }
                }
            }

            if (extendedSettings != null) {
                var editor = CreateEditor(extendedSettings);
