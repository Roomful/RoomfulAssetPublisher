...
namespace net.roomful.assets.editor
{
    ...
    {
        private const string MENU_ROOT = "Roomful/Assets Publisher/";
        //--------------------------------------
        //  Top Menu
        //--------------------------------------

        [MenuItem(MENU_ROOT+ "Wizzard &w", false, 1005)]
        ...
        [MenuItem(MENU_ROOT+ "Create/Avatar Position Marker &a", false, 0)]
        public static void ShowWizzardAvatarPositionMarker() {
            AddAvatarPositionMarker();
        }
        //--------------------------------------
        //  Context Menu
        //--------------------------------------
        [MenuItem("GameObject/Roomful Assets/Avatar Position Marker/Create", false, 0)]
        private static void AddAvatarPositionMarker() {
            var parent = Selection.activeTransform;
            if (parent == null) {
                return;
            }
            var marker = new GameObject("AvatarPositionMarker").AddComponent<AvatarPositionMarker>();
        ...
        [MenuItem("GameObject/Roomful Assets/Avatar Position Marker/Select All", false, 0)]
        private static void SelectAllAvatarPositionMarkers() {
            var markers = new List<Object>();
            foreach (var root in SceneManager.GetActiveScene().GetRootGameObjects()) {
                foreach (var marker in root.GetComponentsInChildren<AvatarPositionMarker>()) {
                    markers.Add(marker.gameObject);
                }
            }
            Selection.objects = markers.ToArray();
        }
