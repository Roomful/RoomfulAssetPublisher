...
namespace SA.CrossPlatform.Editor
{
    ...
    class UM_DefinesResolver : SA_PluginInstallationProcessor<UM_Settings>
    {
        ...
        static void UpdateSdkDefine(bool enabled, string define)
        {
            if (enabled)
            {
                if (!EditorDefines.HasCompileDefine(define)) EditorDefines.AddCompileDefine(define);
            }
            else
            {
                if (EditorDefines.HasCompileDefine(define)) EditorDefines.RemoveCompileDefine(define);
            }
        }
    }
}
